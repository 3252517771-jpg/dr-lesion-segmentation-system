...
IMAGE_SIZE = 512
PATCH_SIZE = 512
PATCHES_PER_EPOCH = 1532
FULL_IMAGE_SAMPLE_PROB = 0.50
LESION_CROP_PROB = 0.50
LESION_SAMPLE_WEIGHTS = {"HE": 1.0, "EX": 1.0, "MA": 2.0, "SE": 1.0}
...
MAX_POS_WEIGHT = 50.0
CLASS_WEIGHT_MULTIPLIER = (1.0, 1.0, 2.0, 1.0)
LOSS_MASK_DILATION = {"HE": 0, "EX": 0, "MA": 1, "SE": 0}
...
print(f"Full image sample probability: {FULL_IMAGE_SAMPLE_PROB}")
print(f"Lesion crop probability: {LESION_CROP_PROB}")
...
def crop_with_padding(
    ...
    image_crop = np.zeros((crop_size, crop_size, image.shape[2]), dtype=image.dtype)
    mask_crop = np.zeros((crop_size, crop_size, mask.shape[2]), dtype=mask.dtype)
    image_crop[dst_y1:dst_y2, dst_x1:dst_x2] = image[src_y1:src_y2, src_x1:src_x2]
    mask_crop[dst_y1:dst_y2, dst_x1:dst_x2] = mask[src_y1:src_y2, src_x1:src_x2]
    return image_crop, mask_crop


def random_crop_origin(width: int, height: int, crop_size: int) -> tuple[int, int]:
    max_x = max(width - crop_size, 0)
    max_y = max(height - crop_size, 0)
    return random.randint(0, max_x), random.randint(0, max_y)


def centered_crop_origin(
    center_x: int,
    center_y: int,
    width: int,
    height: int,
    ...
    jitter = crop_size // 4
    x1 = center_x - crop_size // 2 + random.randint(-jitter, jitter)
    y1 = center_y - crop_size // 2 + random.randint(-jitter, jitter)
    x1 = min(max(x1, 0), max(width - crop_size, 0))
    y1 = min(max(y1, 0), max(height - crop_size, 0))
    return x1, y1


class DDRPatchDataset(Dataset):
    def __init__(
        self,
        base_dataset: DDRLesionDataset,
        transform: A.Compose | None,
        full_transform: A.Compose | None,
        patches_per_epoch: int,
        crop_size: int,
        full_image_prob: float,
        lesion_crop_prob: float,
    ) -> None:
        ...
        self.full_image_prob = full_image_prob
        self.lesion_crop_prob = lesion_crop_prob
        self.image_infos = self._build_image_infos()
        self.class_to_images = {
            lesion: [
                info_index
                for info_index, info in enumerate(self.image_infos)
                if info["pixels"][lesion] > 0
            ]
            for lesion in LESION_CLASSES
        }
        self.lesion_sampling = []
        for lesion in LESION_CLASSES:
            self.lesion_sampling.extend(
                [lesion] * int(LESION_SAMPLE_WEIGHTS.get(lesion, 1.0) * 10)
            )

        print("\nPatch training index")
        print(f"  patches_per_epoch={self.patches_per_epoch}")
        print(f"  crop_size={self.crop_size}")
        print(f"  full_image_prob={self.full_image_prob}")
        print(f"  lesion_crop_prob={self.lesion_crop_prob}")
        for lesion in LESION_CLASSES:
            print(f"  {lesion}: {len(self.class_to_images[lesion])} images with lesion")

    def _build_image_infos(self) -> list[dict[str, object]]:
        infos: list[dict[str, object]] = []
        for image_path in tqdm(self.base_dataset.image_paths, desc="Build patch index", leave=False):
            pixels: dict[str, int] = {}
            for lesion in LESION_CLASSES:
                mask_path = self.base_dataset.get_mask_path(image_path, lesion)
                if mask_path is None:
                    pixels[lesion] = 0
                else:
                    mask = read_binary_mask(mask_path)
                    pixels[lesion] = int(mask.sum())
            infos.append({"image_path": image_path, "pixels": pixels})
        return infos

    def __len__(self) -> int:
        return self.patches_per_epoch

    def _load_full_sample(self, image_path: Path) -> tuple[np.ndarray, np.ndarray]:
        image = read_rgb_image(image_path)
        height, width = image.shape[:2]
        masks = []
        for lesion in LESION_CLASSES:
            ...
            if mask_path is None:
                ...
                if mask.shape[:2] != (height, width):
                    mask = cv2.resize(mask, (width, height), interpolation=cv2.INTER_NEAREST)
            masks.append(mask)
        return image, np.stack(masks, axis=-1).astype(np.float32)

    def _choose_lesion_image(self) -> tuple[Path, str] | None:
        lesion_candidates = [
            lesion
            for lesion in self.lesion_sampling
            if self.class_to_images.get(lesion)
        ]
        if not lesion_candidates:
            return None
        lesion = random.choice(lesion_candidates)
        info_index = random.choice(self.class_to_images[lesion])
        return self.image_infos[info_index]["image_path"], lesion

    def __getitem__(self, index: int) -> tuple[torch.Tensor, torch.Tensor]:
        if random.random() < self.full_image_prob:
            info = random.choice(self.image_infos)
            image, mask = self._load_full_sample(info["image_path"])
            transform = self.full_transform or valid_transform
        ...
        use_lesion_crop = random.random() < self.lesion_crop_prob
        selected = self._choose_lesion_image() if use_lesion_crop else None
        if selected is None:
            info = random.choice(self.image_infos)
            image_path = info["image_path"]
            focus_lesion = None
        else:
            image_path, focus_lesion = selected

        image, mask = self._load_full_sample(image_path)
        height, width = image.shape[:2]

        if focus_lesion is None:
            x1, y1 = random_crop_origin(width, height, self.crop_size)
        else:
            class_index = LESION_CLASSES.index(focus_lesion)
            ys, xs = np.where(mask[:, :, class_index] > 0)
            if len(xs) == 0:
                x1, y1 = random_crop_origin(width, height, self.crop_size)
            else:
                point_index = random.randrange(len(xs))
                x1, y1 = centered_crop_origin(
                    int(xs[point_index]),
                    int(ys[point_index]),
                    width,
                    height,
        ...
        image_crop, mask_crop = crop_with_padding(image, mask, x1, y1, self.crop_size)
        ...
        if self.transform is not None:
            ...
            image_tensor = torch.from_numpy(image_crop.transpose(2, 0, 1)).float() / 255.0
            mask_tensor = torch.from_numpy(mask_crop.transpose(2, 0, 1)).float()
...
train_full_transform = A.Compose(
    [
...
train_base_dataset = DDRLesionDataset("train", transform=None)
...
train_mask_stats = inspect_masks(train_base_dataset)
...
POS_WEIGHT = compute_resized_pos_weight(train_base_dataset).to(device)

train_dataset = DDRPatchDataset(
    base_dataset=train_base_dataset,
    transform=train_transform,
    full_transform=train_full_transform,
    patches_per_epoch=PATCHES_PER_EPOCH,
    crop_size=PATCH_SIZE,
    full_image_prob=FULL_IMAGE_SAMPLE_PROB,
    lesion_crop_prob=LESION_CROP_PROB,
)
...
for epoch in range(1, EPOCHS + 1):
    ...
    epoch_record = {
        ...
        "patches_per_epoch": PATCHES_PER_EPOCH,
        "full_image_sample_prob": FULL_IMAGE_SAMPLE_PROB,
        "lesion_crop_prob": LESION_CROP_PROB,
        "lesion_sample_weights": LESION_SAMPLE_WEIGHTS,
        "batch_size": BATCH_SIZE,
